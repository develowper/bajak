...
<template>
  <div v-if="type == 'checkbox'" class="relative flex justify-end items-center" dir="ltr">
    <label
      class="inline-block mx-2 hover:cursor-pointer text-gray-500 dark:text-slate-100"
      :for="id"
    >
      {{ placeholder }}
    </label>
  ...
  <div v-else>
    ...
    <div class="flex flex-wra items-stretch">
      <span
        v-if="$slots.prepend"
        class="flex bg-gray-100 text-gray-500 items-center whitespace-nowrap rounded-s border border-e-0 border-solid border-slate-300 text-center text-base font-normal leading-[1.6] dark:border-slate-600 dark:text-slate-500 dark:placeholder:text-slate-500"
        id="basic-addon1"
      >
          ...
          ($slots.append && $slots.prepend
            ...
            : $slots.append
              ...
              : $slots.prepend
                ...
                : ' rounded ')
        "
        class="flex grow w-full border border-solid border-slate-300 px-3 text-slate-700 transition duration-200 ease-in-out focus:z-[3] focus:border-primary focus:text-slate-700 dark:border-slate-600 dark:focus:border-primary"
        :value="modelValue"
      ...
      <span
        v-if="$slots.append"
        class="flex bg-gray-100 items-center whitespace-nowrap rounded-e border border-s-0 border-solid border-slate-300 px-3 py-[0.25rem] text-center text-base font-normal leading-[1.6] text-slate-700 dark:border-slate-600 dark:text-slate-500 dark:placeholder:text-slate-500"
        id="basic-addon2"
      >
